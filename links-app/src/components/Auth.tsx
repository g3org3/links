...
export default function AuthComponent() {
  const [_, setProfile] = useState<RecordAuthResponse<Record> | undefined>(
    undefined
  );
    ...
    return (
      <div className="mx-2 flex flex-col items-center gap-4 self-center bg-white p-5">
        <h1 className="text-4xl">
          Welcome {client.current.authStore.model?.email}!
        </h1>
        <form className="flex" onSubmit={onAuthClient}>
          <input
            className="rounded-l border px-4 py-2"
            placeholder="client id"
            name="clientid"
          />
          <button className="rounded-r bg-blue-600 px-4 py-2 text-white">
            authenticate client
          </button>
        </form>

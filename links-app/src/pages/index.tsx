...
const Home: NextPage = () => {
  ...
  const { isFetching: isSearching, data: searchResults } =
    trpc.example.search.useQuery(
      { query: search },
    ...
    trpc.example.linksp.useInfiniteQuery(
      { limit: 16 },
      {
        ...
        onSuccess(linksbypage) {
          const isDiff =
            storelinks[0]?.id !== linksbypage.pages.at(0)?.items[0]?.id;
  ...
  const isEndReached =
    data?.pages && data?.pages[0]?.totalPages === data?.pages.length;
    ...
    <>
      ...
      >
        {(isLoading || isSearching) &&
          new Array(16).fill(0).map((_, x) => <Loading key={x} />)}
        {links.map((link, i) => (
          <LinkCard
            key={link.url}
            link={link}
            cardRef={
              i === links.length - 1 && !isEndReached && !search ? ref : null
            }
          />
        ))}
        {!!search && links.length === 0 && (
          <div className="mt-20 w-full text-center text-3xl text-white opacity-75">
            no search results
          </div>
        )}
        {isEndReached && (
